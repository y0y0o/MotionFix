"""
MotionFix V11 - Test Script

V11 改进点:
  - 输出后加轻量时序平滑（3帧窗口），去除残留高频抖动
...
#  后处理: 轻量时域平滑（V11 新增）
# ================================================================
def temporal_smooth(motion, window=3):
    """
    对每个关节每个维度做滑动窗口平均，去除残留高频抖动。
    motion: (T, 22, 3)  numpy
    返回:   (T, 22, 3)  numpy — 平滑后的动作
    """
    smoothed = motion.copy()
    for j in range(motion.shape[1]):
        for d in range(motion.shape[2]):
            smoothed[:, j, d] = uniform_filter1d(
                motion[:, j, d], size=window, mode='nearest'
            )
    return smoothed
...
def fix_motion(model, motion, device='cuda'):
    ...
    # V11: 轻量时域平滑，去除残留高频抖动
    fixed = temporal_smooth(fixed, window=3)
